...
import { totalCycles, daysCap, sortedReviews, reviewLabel, reviewKey } from "../utils/lifecycle";
  ...
  return (
    <div className="border-b border-slate-50 last:border-0">
      ...
      >
        ...
        <Tag className="bg-slate-100 text-slate-600 text-xs shrink-0">{reviewLabel(rv)}</Tag>
...
export default function EmployeeProfile({ rec, editable, onSaveProfile, compact = false, showContact = true, showCompliance = true, showReviewHistory = true }) {
    ...
    <div className={compact ? "space-y-3" : "space-y-4"}>
      ...
      <div className={`grid md:grid-cols-2 ${compact ? "gap-3" : "gap-4"}`}>
        ...
        <div className={compact ? "space-y-3" : "space-y-4"}>
          ...
          {/* Review history */}
          {showReviewHistory && rec.reviews?.length > 0 && (
            <Card className={`${compact ? "p-3" : "p-4"} overflow-hidden`}>
              ...
              <div className="space-y-0">
                {sortedReviews(rec.reviews).map((rv) => (
                  <ProfileReviewRow key={reviewKey(rv)} rv={rv} kpis={rv.kpisSnapshot || kpisForCycle(rec, rv.cycle)} />
                ))}
              </div>
            </Card>
          )}
          ...
          <Card className={compact ? "p-3" : "p-4"}>
            ...
            {rec.completion ? (
              ...
              <div className="flex items-center gap-3 py-3">
                <div className="grid place-items-center w-9 h-9 rounded-full shrink-0 bg-amber-50 text-amber-500">
                  <Clock size={16} />
                </div>
                <p className="text-sm text-slate-400">
                  {rec.letterId ? "Not Signed Yet." : "Awaiting letter generation."}
                </p>
              </div>
            )}
            {rec.hrbpAckRemarks && (
