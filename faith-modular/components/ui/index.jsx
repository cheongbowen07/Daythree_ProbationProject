...
import { ROLES, TONE_CLASS, statusLabel, tone } from "../../constants";
...
export function Mono({ children, className = "" }) { return <span className={className} style={{ fontFamily: "var(--mono)" }}>{children}</span>; }
...
export function Tag({ children, className = "" }) { return <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-[11px] font-medium ${className}`} style={{ fontFamily: "var(--mono)" }}>{children}</span>; }
...
export function Card({ children, className = "" }) { return <div className={`rounded-lg bg-white ring-1 brand-card ${className}`}>{children}</div>; }
...
export function Row({ k, v }) { return <div className="flex justify-between gap-3"><dt className="text-slate-400">{k}</dt><dd className="text-slate-700 text-right">{v}</dd></div>; }
